<aside
    class="w-72 flex-col p-5 lg:flex hidden  bg-slate-200 h-screen sticky top-0 border-l border-gray-400 text-gray-500"
    ...
    <ul class="">
        ...
        @php
        $url = request()->url();
        $open = Str::contains($url, ['Delivery','Payment','admin/showOrderList', 'admin/order/edit',
        'admin/order/create', 'admin/order/store']);
        @endphp
        ...
        <li x-data="{ open: {{ $open ? 'true' : 'false' }} }" class="mb-1 text-sm">
            <button @click="open = !open" class="flex items-center w-full p-2 gap-2 rounded hover:bg-slate-300">
                ...
                <svg :class="{ 'rotate-90': open }" x-transition class="mr-auto -mr-1 h-5 w-5 text-gray-400"
        ...
        @php
        $url = request()->url();
        $openReportMenu = Str::contains($url,
        ['admin/orderReport','admin/cashReport','admin/billReportView','admin/createBill']);
        @endphp
...
<div class="navbar-menu relative z-50 hidden  ">
    ...
    <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm   bg-white border-r overflow-y-auto">
        ...
        <div class="  flex-col p-5  bg-slate-200  sticky   border-l border-gray-400 text-gray-500"
            @click.outside="sidebarToggle = false">
            <span class="flex items-center flex-col mb-5">
                <div class="flex  place-self-start border mb-8">
         ...
         hover:bg-slate-300">
                ...
                ['admin/orderReport','admin/cashReport','admin/billReportView','admin/createBill']);
                @endphp
                <li x-data="{ open: {{ $openReportMenu ? 'true' : 'false' }}  }" class="mb-1 text-sm"
                    x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center w-full p-2 gap-2 rounded hover:bg-slate-300">
                ...
                $openJobsMenu = Str::contains($url, ['admin/Jobs','admin/JobApp']);
                @endphp
                <li x-data="{ open: {{ $openJobsMenu ? 'true' : 'false' }} }" class="mb-1 text-sm"
                    x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center w-full p-2 gap-2 rounded hover:bg-slate-300">
                ...
                $openJobsPerm = Str::contains($url, ['admin/perm','indexrole','admin/usersList','admin/Users/create']);
                @endphp
                <li x-data="{ open: {{ $openJobsPerm ? 'true' : 'false' }} }" class="mb-1 text-sm"
                    x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center w-full p-2 gap-2 rounded hover:bg-slate-300">
                ...
                @endphp
                @can('Setting')
                <li x-data="{ open: {{ $openSetting ? 'true' : 'false' }} }" class="mb-1 text-sm"
                    x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center w-full p-2 gap-2 rounded hover:bg-slate-300">
